# Catalyst Function Samples

Paste these code blocks into the matching Catalyst Basic I/O functions:
...
These functions are designed for the current `collateral_entries` Data Store table and the frontend requests in `src/lib/catalyst-collateral-api.ts`.
...
module.exports = async (context, basicIO) => {
  try {
    ...
    if ((duplicate || []).length) {
      ...
      basicIO.write(
        JSON.stringify({ error: `This link already exists for ${existingRecord.asset_name}.` }),
      );
      context.close();
      return;
    }
...
module.exports = async (context, basicIO) => {
  try {
    ...
    const duplicate = await app.zcql().executeZCQLQuery(
      `SELECT ROWID, asset_name FROM collateral_entries WHERE link = '${sanitizeSql(link)}' AND ROWID != '${sanitizeSql(id)}' LIMIT 1`,
    );
    ...
    basicIO.write(JSON.stringify(toRecord(updatedRow)));
    context.close();
  } catch (error) {
    basicIO.setStatus(500);
    basicIO.write(
      JSON.stringify({
        error: 'Failed to update collateral in Catalyst',
        details: error.message || String(error),
      }),
    );
    context.close();
